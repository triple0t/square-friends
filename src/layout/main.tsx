...
const Main = () => {
  ...
  useEffect(() => {
    const url = window.location.pathname.includes('square-friends') ? '/square-friends/friendslist.json' : '/friendslist.json';
    fetch(url)
      .then((res) => res.json())
    ...
    <main className="bg-gray-50 dark:bg-black">
      <div className="max-w-6xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="py-4 px-4 flex items-center justify-between h-16">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('friends_list')}</h1>
