...
const FriendsListCard: FunctionComponent<FriendsListCardProps> = ({
  profileInfo,
  ...
  return (
    <div className="bg-white shadow overflow-hidden rounded-lg h-auto sm:h-80 md:h-64 my-4 dark:bg-gray-850">
      <img
        src={profileInfo?.header_image}
        alt={`Header_image for ${profileInfo?.name}`}
                ...
                <span
                  className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    isFollowingCheck
                      ...
                      : "text-blue-450 border border-blue-450 dark:text-white dark:border-white"
                  }`}
                >
                  {isFollowingCheck ? t("following") : t("follow")}
                </span>
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-440">
                {profileInfo?.username}
              </p>
            </div>
            <p className="text-gray-900 dark:text-gray-420">
              {" "}
              {profileInfo?.bio}{" "}
            </p>
            <br />
